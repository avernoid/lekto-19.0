...
class PleUpdateTagsWizard(models.TransientModel):
    _name = 'ple.update.tags.wizard'
    _description = 'Wizard to Manual Update PLE Tags'

    company_id = fields.Many2one('res.company', string='Company', required=True, default=lambda self: self.env.company)
    config_ids = fields.Many2many('ple.sale.tax.config', string='Configurations to Apply', required=True)
    ...
    def action_execute(self):
        ...
        for config in self.config_ids:
            # Construct XML ID: e.g. l10n_pe.1_sale_tax_igv_18
            xml_id = "%s.%s_%s" % (config.tax_prefix, self.company_id.id, config.tax_code)
