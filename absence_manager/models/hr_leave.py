...
class HrLeave(models.Model):
    ...
    hr_attendance_ids = fields.One2many(
        ...
        string='Attendances',
        help="Attendance records automatically generated when this leave is approved. "
             "These records are deleted if the leave is refused.",
    )
    report_attendance = fields.Boolean(
        string='Report in Attendance?',
        help="When enabled and the leave is approved, attendance records will be "
             "automatically created for each day of the leave period. If the leave "
             "is later refused, these attendance records will be removed. Leaves "
             "generated by the Absence Monitor CRON have this disabled by default.",
        default=False,
    )
    ...
    def _onchange_employee_attendance(self):
        for rec in self:
            rec.report_attendance = rec.employee_id.attendance if rec.employee_id else False
    ...
    def get_hr_leave_values(self, line_morning, line_afternoon, str_name, employee):
        hr_leave_type_id = self.env.ref('absence_day.hr_leave_type_ppd', False)
    ...
    def action_back_to_approval(self):
        super(HrLeave, self).action_back_to_approval()
        for rec in self:
            if rec.report_attendance:
                rec.hr_attendance_ids.unlink()