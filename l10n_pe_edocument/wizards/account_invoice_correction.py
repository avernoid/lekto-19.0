...
class AccountInvoiceCorrection(models.TransientModel):
    ...
    journal_id = fields.Many2one(
        string='Specific Journal',
        comodel_name='account.journal',
        domain=[('type', '=', 'sale'), ('l10n_latam_use_documents', '=', False)],
        help='If empty, it uses the journal of the journal entry to make the charge.'
    )
    ...
    @staticmethod
    def _prepare_account_move_line(move_line):
        return {
            'name': move_line.name,
    ...
    def create_invoice_correction(self):
        """
        This method creates a correction invoice based on the selected invoice.
        """
        self.ensure_one()
        move = self.move_id
        invoice_line_list = []
        ...
        move_msg = _("This correction invoice was created from:") + " <a href=# data-oe-model=account.move data-oe-id=%d>%s</a>" % (move.id, move.name)
        new_move.message_post(body=move_msg)
        move_to_redirect |= new_move

        action = {
            'name': _('Correction Invoice'),
