...
class TestGithubConnectorAnalysisRuleBase(TestGithubConnectorCommon):
    ...
    def setUpClass(cls):
        ...
        cls.rule_group = cls.env.ref(
            "github_connector_api.github_analysis_rule_group_1_demo", raise_if_not_found=False
        )
        if not cls.rule_group:
            cls.rule_group = cls.env["github.analysis.rule.group"].create({
                "name": "Demo Group",
            })
        cls.rule_python = cls.env.ref("github_connector_api.github_analysis_rule_python", raise_if_not_found=False)
        if not cls.rule_python:
            cls.rule_python = cls.env["github.analysis.rule"].create({
                "name": "Python",
                "group_id": cls.rule_group.id,
                "paths": "*.py",
            })
        cls.rule_xml = cls.env.ref("github_connector_api.github_analysis_rule_xml", raise_if_not_found=False)
        if not cls.rule_xml:
            cls.rule_xml = cls.env["github.analysis.rule"].create({
                "name": "XML",
                "group_id": cls.rule_group.id,
                "paths": "*.xml",
            })
        cls.rule_js = cls.env.ref("github_connector_api.github_analysis_rule_js", raise_if_not_found=False)
        if not cls.rule_js:
            cls.rule_js = cls.env["github.analysis.rule"].create({
                "name": "JS",
                "group_id": cls.rule_group.id,
                "paths": "*.js",
            })
        cls.rule_test = cls.env.ref("github_connector_api.github_analysis_rule_test", raise_if_not_found=False)
        if not cls.rule_test:
            cls.rule_test = cls.env["github.analysis.rule"].create({
                "name": "Test",
                "group_id": cls.rule_group.id,
                "paths": "tests/*",
            })
        ...
        # _set_github_responses will be called by _download_and_analyze
    ...
    def _set_github_responses(self, rsps):
        for github_id in self.oca.mapped("repository_ids.github_id_external"):
            with open(
                file_path(
                    "github_connector_api/tests/res/github_repo_%s_response.json" % github_id
                )
            ) as jsonfile:
                rsps.add(
                    responses.GET,
