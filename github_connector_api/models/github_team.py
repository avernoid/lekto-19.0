...
class GithubTeam(models.Model):
    ...
    is_review_team = fields.Boolean(string="Is team reviewer?")

    @api.constrains('is_review_team')
    def constrains_is_review_team(self):
        if self.search([('is_review_team', '=', True)], count=True) > 1:
            raise UserError(_("There is already a selected review team, update configuration."))
    ...
    def get_github_base_obj_for_creation(self):
        return self.get_github_connector()

    def create_in_github(self, **kwargs):
        """Create an object in Github through the API"""
        # Create in Github
        gh_api = self.get_github_base_obj_for_creation()
        gh_base_obj = gh_api.get_organization(kwargs.get("github_name"))

        gh_team = gh_base_obj.create_team(
            name=kwargs.get("team_name"),
            description=kwargs.get("team_description") or "",
            privacy=kwargs.get("team_privacy"),
        )

        if kwargs.get("team_members"):
            list(map(lambda member: gh_team.add_membership(gh_api.get_user(member.github_name), role="member"), kwargs.get("team_members")))
        if kwargs.get("team_repositories"):
            list(map(lambda repo: gh_team.add_to_repos(gh_api.get_repo(repo.github_name)), kwargs.get("team_repositories")))
    ...
    def action_github_team_partner_from_team(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "github_connector_api.action_github_team_partner_from_team"
        )
    ...
    def action_github_team_repository_from_team(self):
        self.ensure_one()
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "github_connector_api.action_github_team_repository_from_team"
        )
        action["context"] = dict(self.env.context)
        action["context"].pop("group_by", None)
        action["context"]["search_default_team_id"] = self.id
