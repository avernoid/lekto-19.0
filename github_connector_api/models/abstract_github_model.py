...
class AbstractGithubModel(models.AbstractModel):
    ...
    github_last_sync_date = fields.Datetime(
        string="Last Sync Date with Github", readonly=False
    )
    ...
    def process_timezone_fields(self, res):
        for k, v in res.items():
            if self._fields[k].type == "datetime" and v:
                # Si viene como cadena, intentar parsear formatos ISO comunes
                if isinstance(v, str):
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                # No se pudo parsear; omitir para evitar excepciones
                                _logger.debug("No se pudo parsear la fecha %s para el campo %s", v, k)
                                continue
                    v = dt
                # Asegurarse de que el datetime sea ingenuo (sin tzinfo)
    ...
    def get_odoo_data_from_github(self, data):
        """Mapea un objeto/dict de Github a un diccionario de valores listo para Odoo."""
        map_dict = self.get_conversion_dict()
        res = {}
        is_dict = isinstance(data, dict)
        for k, v in map_dict.items():
            # Validar que el campo Odoo exista en el modelo
            if k not in self._fields:
                continue
            # Obtener valor desde dict o desde atributo del objeto
            value = data.get(v) if is_dict else getattr(data, v, None)
            if value is None:
                continue
            res[k] = value

        # res["github_last_sync_date"] = fields.Datetime.now()
    ...
    def get_from_id_or_create(self, gh_data=None, data=None, extra_data=None):
        ...
        >>> self.env['github_organization'].get_from_id_or_create(
            data={'id': 7600578, 'url': 'https://api.github.com/orgs/odoopartners'})
        """
        ...
        #     github_connector_api = self.get_github_connector(self.github_type())
        #     data = github_connector_api.get_by_url(data["url"], "get")
    ...
    def create_from_name(self, name):
        ...
        >>> self.env['github_organization'].create_from_name('odoopartners')
        >>> self.env['github_repository'].create_from_name('odoopartners/web')
        """
    ...
    def get_github_token(self):
        ...
        github_access_token = ICP.get_param("github.access_token")
    ...
    def get_github_connector(self):
        ...
        if github_access_token:
            try:
                token = Auth.Token(github_access_token)
                return Github(auth=token)
            except Exception as e:
                raise UserError(
                    _("Se produjo un error al autenticar con GitHub: {}".format(str(e)))
                )
