...
class GithubRepository(models.Model):
    ...
    mb_size = fields.Float(string="Size (Megabyte)", readonly=True)
    ...
    coverage_url = fields.Char(string="Coverage URL", store=True, compute="_compute_coverage_url")
    ci_url = fields.Char(string="CI URL", store=True, compute="_compute_ci_url")
    github_url = fields.Char(string="Github URL", store=True, compute="_compute_github_url")
    ...
    def _get_source_path(self):
        return self.env["ir.config_parameter"].get_param("github_source_code_local_path")
    ...
    def button_download_as_zip_repository(self):
        self.ensure_one()
        self._download_code()
        url = '/web/binary/download_github_repository?repo_path=%s&repo_name=%s&repo_branch=%s' % (self.local_path, self.repository_id.name, self.name)
        return {
    ...
    def _download_code(self):
        for branch in self:
            ...
            github_token = repository.get_github_token()
            clone_url = gh_repo.clone_url

            if getattr(gh_repo, 'private', False) and github_token:
                # https://github.com/usuario/repo.git -> https://<token>@github.com/usuario/repo.git
            ...
            if not os.path.exists(branch.local_path):
                ...
                os.system(command)
                repo_data = {"last_download_date": datetime.today(), "state": "to_analyze"}
                repo_data.update(branch.analyze_code_one())
                branch.write(repo_data)
            else:
                # Update repository
                _logger.info("Pulling existing repository %s ..." % branch.local_path)
                try:
                    res = check_output(["git", "pull", "origin", branch.name], cwd=branch.local_path)
                    vals = {"last_download_date": datetime.today()}
                    if b"up to date" not in res:
                        vals["state"] = "to_analyze"
                    vals.update(branch.analyze_code_one())
                    branch.write(vals)
                except Exception:
                    raise exceptions.UserError(
                        _(
    ...
    def analyze_code_one(self):
        ...
        try:
            ...
            return {"size": 0, 'mb_size': 0.0}
        return {"size": size, 'mb_size': float(size) / (1024 ** 2)}
    ...
    def _compute_complete_name(self):
        for branch in self:
            if branch and branch.repository_id:
    ...
    def _compute_local_path(self):
        ...
        if not source_path and not tools.config["test_enable"]:
            raise exceptions.UserError(
                _(
                    "github_source_code_local_path should be defined in your "
                    " configuration file"
                )
            )
        for branch in self:
