...
class AccountChartTemplate(models.AbstractModel):
    ...
    def ple_purchase_book_post_init(self):
        try:
            all_companies = self.env['res.company'].search([('account_fiscal_country_id.code', '=', 'PE')])
            for company in all_companies:

                existing_taxes = self.env['account.tax'].search([
                    ('company_id', '=', company.id),
                    ('name', 'in', [
                        'Valor Ref', '18% IGV - DUA', 'CIF', 
                        'IGV 18% Crédito No Domic.', 'Percepción DUA',
                        'Ad Valorem', 'IGV 18% No Domic.', '30% Ret No Domic.'
                    ])
                ])
                if existing_taxes:
                    continue
