...
class TestBaseSpot(TransactionCase):
    ...
    def test_set_base_spot(self):
        ...
        vals = account_move_model.search([('move_type', 'in', ['in_invoice', 'in_refund'])], order='id desc')
        self.assertEqual(2, len(vals))
        self.assertEqual(account_move_unit1, vals[1])
        self.assertEqual(account_move_unit2, vals[0])
        vals = account_move_model.search([('detraction_id', 'in', [account_spot_detraction[0].id, account_spot_detraction[1].id])], order='id desc')
        self.assertEqual(2, len(vals))
        self.assertEqual(account_move_unit1, vals[1])
        self.assertEqual(account_move_unit2, vals[0])
