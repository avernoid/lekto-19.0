...
#   def _format_value(self, options, value, currency=None, blank_if_zero=False, figure_type=None, digits=1):
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        def _update_column(line, column_key, new_value, blank_if_zero=False):
            line['columns'][column_key]['name'] = report._format_value(options, new_value, figure_type='monetary')
            ...
            if blank_if_zero is not None:
                line['columns'][column_key]['blank_if_zero'] = blank_if_zero
        ...
        def _insert_balance_values(balance_key, debit_index, credit_index):
            ...
            if difference > 0:
                ...
                result_exercise_period_columns.insert(debit_index, {
                    'name': report._format_value(options, 0.0, figure_type='monetary'),
                    'no_format': 0.0,
                    'class': 'number',
                    'blank_if_zero': False
                })
                result_exercise_period_columns.insert(credit_index, {
                    'name': report._format_value(options, difference, figure_type='monetary'),
            ...
            result_totals_columns.insert(debit_index, {
                'name': report._format_value(options, totals_debit, figure_type='monetary'),
                'no_format': totals_debit,
                'class': 'number',
                'blank_if_zero': False
            })
            result_totals_columns.insert(credit_index, {
                'name': report._format_value(options, totals_credit, figure_type='monetary'),
                'no_format': totals_credit,
                'class': 'number',
                'blank_if_zero': False
            }) 

        # Get lines from General Ledger report
        gl_report = self.env.ref('account_reports.general_ledger_report')
        gl_options = options.copy()
        gl_options['report_id'] = gl_report.id
        gl_options['hierarchy'] = False
        # We pass None for all_column_groups_expression_totals so it recomputes them for GL expressions
        lines = gl_report._get_lines(gl_options, all_column_groups_expression_totals=None, warnings=warnings)
        total_diff_values = {
            'initial_balance': 0.0,
            'end_balance': 0.0,
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        # Initial balance
        initial_balance_options = self._get_options_initial_balance(options)
        ...
        # All the periods displayed need to use the same rates, so we manually change the period key.
        # account.report will then compute the currency table periods accordingly
        middle_periods_period_key = '_checkout_balance_middle_periods'
    ...
    def _custom_line_postprocessor(self, report, options, lines, warnings=None):
        ...
        for line in lines:
            ...
            if model == 'account.group':
                ...
                # 2. If line is manually unfolded by user (in unfolded_lines), force expand.
                # 3. Otherwise, force collapse (this ensures it is deactivated by default).
                if unfold_all:
                    line['unfolded'] = True
                elif line['id'] in unfolded_lines:
                    line['unfolded'] = True
                else:
                    line['unfolded'] = False
                ...
                # Keep generic lines (Totals, Results, etc.)
                filtered_lines.append(line)
            
            # Explicitly drop 'account.move.line' or any other model to prevent Journal Items from showing
    ...
    def _get_options_initial_balance(self, options):
        ...
        new_options['date']['date_from'] = fields.Date.to_string(new_date_to)
