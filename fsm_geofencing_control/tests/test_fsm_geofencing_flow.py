...
# from unittest.mock import patch  # Removed as we don't need to mock BaseGeocoder anymore
...
class TestFSMGeofencingFlow(TransactionCase):
    ...
    def setUp(self):
        ...
        # Mock _get_localisation was removed because Odoo 18 logic (and our module)
        # relies on 'geolocation' context passed from client/controller, 
        # not on BaseGeocoder internal calls during validation.
    ...
    def test_stop_timer_with_lost_reason(self):
        ...
        # Start timer (we just simulate it started or use method if needed)
        geolocation_start = {
            'success': True,
            'latitude': 40.7128,
            'longitude': -74.0060,
        ...
        # Instead of manually creating timesheet, we use the new wizard which creates it.
        # Create wizard (model: project.task.create.timesheet)
        ...
        # Should not raise error
        wizard.with_context(geolocation=geolocation_stop).save_timesheet()
    ...
    def test_auto_mark_done(self):
        ...
        # Save timesheet
        result = wizard.with_context(geolocation=geolocation_stop).save_timesheet()
