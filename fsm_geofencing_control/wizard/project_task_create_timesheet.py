...
class ProjectTaskCreateTimesheet(models.TransientModel):
    ...
    control_distance_on_stop = fields.Boolean(
        related="task_id.project_id.control_distance_on_stop",
        readonly=True
    )
    
    allow_geolocation = fields.Boolean(
        related="task_id.project_id.allow_geolocation",
        readonly=True
    )

    
    use_lost_reason = fields.Boolean(
        related="task_id.project_id.use_lost_reason",
        readonly=True
    )
    
    auto_mark_done = fields.Boolean(
        related="task_id.project_id.auto_mark_done",
        readonly=True
    )
