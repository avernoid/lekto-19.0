...
class SirePurchaseBase(models.AbstractModel):
    ...
    company_id = fields.Many2one(
        ...
        help='Company for which the SIRE purchase report will be generated.',
    )
    year = fields.Selection(
        selection=lambda self: [
            (str(y), str(y)) for y in reversed(range(2010, fields.Date.today().year + 2))
        ],
        string='Year',
        required=True,
        default=lambda self: str(
            fields.Date.today().year - 1 if fields.Date.today().month == 1 else fields.Date.today().year
        ),
        help='Fiscal year for the SIRE report period.',
    )
    month = fields.Selection(
        ...
        string='Period',
        required=True,
        default=lambda self: str(fields.Date.today().month - 1 or 12).zfill(2),
        help='Month of the fiscal period for the SIRE report.',
    )
    xlsx_filename = fields.Char(string='Excel Filename')
    xlsx_binary = fields.Binary(string='Excel Report', help='Download the generated XLSX report for review and audit purposes.')
    zip_filename = fields.Char(string='TXT Filename')
    zip_binary = fields.Binary(string='TXT Report', help='Download the generated TXT file ready for upload to the SUNAT SIRE platform.')
    error_dialog = fields.Text(string='Errors', readonly=True, help='Displays any errors encountered during report generation.')
    ...
    def _select_sire_purchase(self):
        return f"""
            ...
            LEFT(COALESCE(TO_CHAR(account_move.l10n_pe_detraction_date, 'DD/MM/YYYY'), ''), 10) AS voucher_payment_date,
            CASE 
            ...
            LEFT(COALESCE(account_move.l10n_pe_detraction_number, ''), 24) AS voucher_number,
            ...
            CASE 
                WHEN currency.name = 'PEN' THEN 
                    NULL 
                ELSE 
                    ROUND(CASE WHEN account_move.invoice_currency_rate > 0 THEN 1 / account_move.invoice_currency_rate ELSE 1 END, 3) 
    ...
    def _process_query_results(self):
        ...
        # Build a map of move_id : prioritized document number (l10n_latam_document_number if available, else ref)
        move_ids = [r.get('move_id') for r in query_results if r.get('move_id')]
        move_doc_map = {}
        if move_ids:
            Move = self.env['account.move']
            has_latam_doc = 'l10n_latam_document_number' in Move._fields
            moves = Move.browse(move_ids).exists()     
            if has_latam_doc:
                move_doc_map = {
                    m.id: ((m.l10n_latam_document_number or '').strip()
                           if getattr(m.journal_id, 'l10n_latam_use_documents', False)
                           else '')
                    for m in moves
                }
            else:
                move_doc_map = {m.id: '' for m in moves}
        for line_result in query_results:
            ...
            if isinstance(country_name, dict):
                if country_name:
                    ...
                    new_country_name = country_name.get(last_key) or ''
                else:
                    new_country_name = ''
            
            # Apply priority for Serie/Nro CP:
            # 1st: l10n_latam_document_number (ORM), 2nd: ref (SQL), 3rd: name (SQL)
            doc_number = (move_doc_map.get(line_result.get('move_id')) or '').strip()
            move_name = (line_result.get('move_name') or '').strip()
            raw_ref = (line_result.get('ref') or '').strip()
            prioritized = doc_number or raw_ref or move_name
            prioritized_clean = prioritized.replace(' ', '').strip()
            line_result['ref'] = prioritized_clean

            if '-' in prioritized_clean:
                serie, correlative = [p.strip() for p in prioritized_clean.split('-', 1)]
            else:
                serie, correlative = '', prioritized_clean.strip()

            doc_code = (line_result.get('document_type_code') or '').strip()
            serie = (serie or '')[:20]
            correlative = (correlative or '')[:20]
            if doc_code == '46' and serie:
                serie = serie.zfill(4)

            line_result['ref_serie'] = serie
            line_result['ref_correlative'] = correlative
