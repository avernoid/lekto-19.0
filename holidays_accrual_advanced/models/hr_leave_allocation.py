...
class HrLeaveAllocation(models.Model):
    ...
    unit_per_interval = fields.Selection(
        ...
        ('hours', 'Hours'),
        ('days', 'Days')], 
        compute='_compute_number_of_days', store=True, string="Unit of time added at each interval",
        readonly=False,
        default=lambda self: self._default_unit_per_interval(),
        help='Select the unit in which the Allocation per Accrual Period is measured. '
             'If set to Hours, the system automatically converts to days using the '
             'employee\'s calendar hours per day (e.g., 12 hours ÷ 8h/day = 1.5 days).',
    )
    ...
    interval_unit = fields.Selection(
        ...
        ('months', 'Month(s)'),
        ('years', 'Year(s)')],
        compute='_compute_accrual_plan_id', store=True, string="Unit of time between two intervals",
        readonly=False,
        default=lambda self: self._default_interval_unit(),
        help='Unit used for the Accrual Period Duration. Combined with the duration number, '
             'this defines how long each accrual period lasts (e.g., 1 Month, 2 Weeks, 1 Year).',
    )
    ...
    accruement_ids = fields.One2many(
        ...
        readonly=True,
        help='Complete audit trail of all balance changes for this allocation. '
             'Each entry records the date, amount, and reason (gains from accrual, '
             'losses from limits). Entries are regenerated on each recalculation.',
    )
    limit_accrued_days = fields.Boolean(
        string='Limit Number of Days Accrued',
        readonly=True,
        help='When enabled, caps the maximum number of days that can be earned '
             'in a single accrual period. Any excess above the limit is recorded '
             'as a "Loss due to accrued amount limit" entry in the audit trail.',
    )
    max_accrued_days = fields.Float(
        string='Max Number of Days Accrued',
        readonly=True,
        help='Maximum number of days that can be accrued in a single period. '
             'Only applies when "Limit Number of Days Accrued" is enabled. '
             'Example: set to 1.5 to cap monthly accrual at 1.5 days even if '
             'the configured allocation per period is higher.',
    )
    limit_carryover_days = fields.Boolean(
        string='Limit Number of Days to Carryover',
        readonly=True,
        help='When enabled, limits how many accumulated days can be carried '
             'over from one period to the next. At the start of each new period, '
             'if the balance exceeds this limit, the excess is trimmed and '
             'recorded as a "Loss due to period carry-over limit" entry.',
    )
    max_carryover_days = fields.Float(
        string='Max Number of Days to Carryover',
        readonly=True,
        help='Maximum number of days that can be carried over between periods. '
             'Only applies when "Limit Number of Days to Carryover" is enabled. '
             'Example: set to 5.0 to ensure no more than 5 days roll over '
             'into the next accrual period.',
    )
    limit_accumulated_days = fields.Boolean(
        string='Limit Total Balance',
        readonly=True,
        help='When enabled, sets an absolute ceiling on the total balance '
             'of accumulated days. Once the balance reaches this limit, no '
             'further accrual is added regardless of periods elapsed. '
             'Excess is recorded as a "Loss due to accumulation limit" entry.',
    )
    max_accumulated_days = fields.Float(
        string='Total Balance Limit',
        readonly=True,
        help='Absolute maximum number of days that can be accumulated across '
             'all periods. Only applies when "Limit Total Balance" is enabled. '
             'Example: set to 30.0 to ensure the total balance never exceeds '
             '30 days regardless of how many periods have passed.',
    )
    accrual_method = fields.Selection(
        ...
        default='prorate',
        readonly=True,
        help='Determines how days are allocated within each period:\n'
             '• Prorate: Days accrued proportionally based on actual worked days '
             'vs. workable days. Employees with unpaid leaves receive less.\n'
             '• Period Start: Full allocation granted on the first day of each period.\n'
             '• Period End: Full allocation granted only after the complete period has elapsed.',
    )
    ...
    number_per_interval = fields.Float(
        ...
        help='Number of days (or hours) that will be accrued in each period. '
             'For example, 20 days/year means the employee earns 20 vacation days '
             'per year. The unit is determined by the "Unit of time" field. '
             'Default: 20 days.',
    )
    interval_number = fields.Integer(
        string='Accrual Period Duration',
        default=lambda self: self._default_interval_number(),
        help='Numeric value for the accrual period length. Combined with the '
             'period unit (Days/Weeks/Months/Years), this defines how often '
             'leave is accrued. Example: "1" with unit "Year(s)" means '
             'accrual is calculated once per year. Default: 1.',
    )
    ...
    def action_recalculate_accrual_allocations(self):
        for allocation in self:
            ...
            if not employee.active_employee:
                continue

            if not employee.contract_date_start:
                continue

            if employee.contract_date_end and employee.contract_date_end < fields.Date.today():
                continue
    ...
    def _update_accrual(self):
        ...
        super(HrLeaveAllocation, allocations.with_context(
            custom_post_accrual=True
        ))._update_accrual()
